$(document).ready(function () {
  var tableData = Array.from(document.querySelector(".table tbody").rows).map(
    (row) => {
      var cells = row.cells;
      var rowData = {};
  ...
  $("#searchForm").on("submit", function (e) {
    ...
    if (!addressSearch.trim()) {
      return; // Exit the function if search term is invalid
    } else {
    ...
    filteredData.forEach((rowData, index) => {
      ...
      if (index % 2 !== 0) {
        // Add the 'odd-row' class to odd rows
